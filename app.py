...
with st.sidebar.expander("Custom Categories"):
    new_category = st.text_input("Add New Category", help="Add a custom category beyond INCLUDE/EXCLUDE")
...
async def screen_video_with_llm(video_data, transcript=None, model_name="gpt-4o-mini"):
    ...
    try:
        ...
        ------------------------------------
        CRUCIAL GUIDANCE:
        - When information is limited, ALWAYS default to INCLUDE, NEVER EXCLUDE
...

        {
          ...
          "reasoning": "Brief explanation emphasizing why included or excluded"
        }
        ...
        try:
            ...
            if "decision" in json_result:
                ...
                if processed_result["decision"] not in ["INCLUDE", "EXCLUDE"]:
                    processed_result["decision"] = "INCLUDE"
            else:
                processed_result["decision"] = "INCLUDE"
...
def debug_llm_responses(df):
   """Analyze why videos are being excluded."""
...
#         - **INCLUDE** ONLY when there is a convergence of strong, positive evidence. Requires meeting the Single-Day Narrative rule AND at least THREE other Inclusion Signals.
...
#             - **Inclusion Signals (at least 2 required):**
...
#         - **EXCLUDE** when ANY of these Exclusion Triggers are present with moderate certainty. A single strong trigger outweighs multiple weak inclusion signals.
#             - **Exclusion Triggers:**
...
# def debug_llm_responses(df):
#     """Analyze why videos are being excluded or marked as 'INCLUDE'."""
#     if 'decision' not in df.columns:
#         st.error("No decision column found in results")
#         return
...
#     # Sample INCLUDE videos
#     if 'INCLUDE' in decisions and decisions['INCLUDE'] > 0:
#         not_sure = df[df['decision'] == 'INCLUDE'].sample(min(5, len(df[df['decision'] == 'INCLUDE'])))
        
#         st.write("### Sample 'INCLUDE' Videos")
...
#                         col2.metric("Include", len(final_df[final_df['decision'] == 'INCLUDE']))
#                         col3.metric("Exclude", len(final_df[final_df['decision'] == 'EXCLUDE']))
#                         col4.metric("INCLUDE", len(final_df[final_df['decision'] == 'INCLUDE']))
...
#                         # Create tabs for all categories including custom ones
#                         all_categories = ["Include", "Exclude", "INCLUDE"] + st.session_state.custom_categories
#                         tabs = st.tabs(all_categories)
...
#                         with tabs[2]:
#                             not_sure_samples = final_df[final_df['decision'] == 'INCLUDE'].sample(min(3, len(final_df[final_df['decision'] == 'INCLUDE'])))
#                             if not not_sure_samples.empty:
#                                 for _, video in not_sure_samples.iterrows():
#                                     display_video_example(video)
#                                     st.divider()
#                             else:
#                                 st.info("No videos were marked as 'INCLUDE'.")
